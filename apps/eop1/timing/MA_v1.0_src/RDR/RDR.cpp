...
void RDR::handleUpdateConfig(json j) {
    Utils::logElapsedTime(j, "RDR", "updateConfig");
...
void RDR::handleLocation(json j) {
    Utils::logElapsedTime(j, "RDR", "pnt");
...
void RDR::handleNewPlan(json j) {
    Utils::logElapsedTime(j, "RDR", "updateMissionPlan");
...
void RDR::handleRequestRDRDetections(json j) {
    Utils::logElapsedTime(j, "RDR", "requestRDRDetections");
	...
	json j1;
	j1["detects"] = resp;
	amq.publish("recieveRDRDetections", j1, true);
}



void RDR::handleGroundMovers(json j) {
    Utils::logElapsedTime(j, "RDR", "groundMovers");
